...
def pivot(data, indexes: list[str] = None, columns: list[str] = None, values: list[str] = None):
    ...
    pivot = pd.pivot_table(
        df, index=indexes, columns=columns, values=values, sort=False, fill_value=0, aggfunc="sum" , margins = True ,  margins_name='Total'
    )
    pivot = pivot.reset_index()
#     total_row = pivot.sum(numeric_only=True)

# # Assign 'Total' to the index columns in the total row
#     for index in indexes:
#         total_row[index] = 'Total'

#     # Convert the total row to a DataFrame and append it to the pivot table
